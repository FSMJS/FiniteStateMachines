...
function draw() {
  ...
  if (keysDown['a']) offsetX = Math.max((offsetX -= moveStep), -((canvasElement.width / 2)));
  if (keysDown['d']) offsetX = Math.min((offsetX += moveStep), ((canvasElement.width / 2))); console.log(offsetX);
